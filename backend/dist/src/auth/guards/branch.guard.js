...
let BranchGuard = class BranchGuard {
    canActivate(context) {
        ...
        if (role === client_1.Role.OWNER) {
            request.branchFilter = null;
            return true;
        }
        if (!user.branchId) {
            throw new common_1.ForbiddenException(`${role} must be assigned to a branch to perform this action`);
        }
        request.branchFilter = user.branchId;
        const requestedBranchId = request.params?.branchId || request.query?.branchId;
        if (requestedBranchId && requestedBranchId !== user.branchId) {
            throw new common_1.ForbiddenException(`${role} can only access their assigned branch`);
        }
        return true;
    }
};
